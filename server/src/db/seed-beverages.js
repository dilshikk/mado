 ...
 * Запуск: node server/src/db/seed-beverages.js
 * Безопасно: использует ON CONFLICT DO NOTHING, повторный запуск не дублирует данные.
 */

import pool from './pool.js';

// ─── Структура: [label_ru, label_uz, label_en, label_tr, tab, dishes[]]
// Блюда: [name_ru, name_uz, name_en, name_tr, price]
...
const CATEGORIES = [
  ...
  {
    label_ru: 'Горячий шоколад',
    label_uz: 'Issiq shokolad',
    label_en: 'Hot Chocolate',
    label_tr: 'Sıcak Çikolata',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Турецкий кофе',
    label_uz: 'Turk qahvasi',
    label_en: 'Turkish Coffee',
    label_tr: 'Türk Kahvesi',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Матча (горячая)',
    label_uz: 'Matcha (issiq)',
    label_en: 'Matcha (Hot)',
    label_tr: 'Matcha (Sıcak)',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Турецкий чай',
    label_uz: 'Turk choy',
    label_en: 'Turkish Tea',
    label_tr: 'Türk Çayı',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Газированные напитки',
    label_uz: 'Gazli ichimliklar',
    label_en: 'Soft Drinks',
    label_tr: 'Gazlı İçecekler',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Кофе со льдом',
    label_uz: 'Muzli qahva',
    label_en: 'Iced Coffee',
    label_tr: 'Buzlu Kahve',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Матча (холодная)',
    label_uz: 'Matcha (sovuq)',
    label_en: 'Matcha (Cold)',
    label_tr: 'Matcha (Soğuk)',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Милкшейки',
    label_uz: 'Milksheyкlar',
    label_en: 'Milkshakes',
    label_tr: 'Milkshake\'ler',
    tab: 'beverage',
    dishes: [
  ...
  {
    label_ru: 'Фреши',
    label_uz: 'Taza sharbatlar',
    label_en: 'Fresh Juices',
    label_tr: 'Taze Meyve Suları',
    tab: 'beverage',
    dishes: [
...
async function seed() {
  ...
  try {
    console.log('🚀 Начинаем загрузку данных напитков...\n');

    let totalCategories = 0;
    let totalDishes = 0;

    for (const [idx, cat] of CATEGORIES.entries()) {
      // Insert category (skip if label_ru already exists)
      ...
      if (catRes.rows.length > 0) {
        ...
        console.log(`✓ Категория [${idx + 1}/${CATEGORIES.length}]: ${cat.label_ru} (id=${categoryId})`);
        totalCategories++;
      } else {
        ...
        if (existing.rows.length === 0) {
          console.log(`⚠ Категория "${cat.label_ru}" не найдена и не создана, пропускаем`);
          continue;
        }
        categoryId = existing.rows[0].id;
        console.log(`~ Категория уже существует: ${cat.label_ru} (id=${categoryId})`);
      }
      ...
      for (const [pos, dish] of cat.dishes.entries()) {
        ...
        await client.query(`
          INSERT INTO dishes
            (category_id, name_ru, name_uz, name_en, name_tr, price, status, position)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
          ON CONFLICT DO NOTHING
        `, [categoryId, name_ru, name_uz, name_en, name_tr, price, status, pos + 1]);

        totalDishes++;
        const priceLabel = price === 0 ? 'черновик' : `${price.toLocaleString('ru-RU')} сум`;
        console.log(`  + ${name_ru} — ${priceLabel}`);
      }
      console.log('');
    }
    ...
    console.log(`   Категорий: ${totalCategories} новых`);
    console.log(`   Блюд:      ${totalDishes} записей`);
    console.log('─'.repeat(50));
  } catch (err) {
