...
router.get('/', async (req, res) => {
  ...
  let query = `SELECT id, title, title_ru, title_uz, title_en, title_tr, slug, status, sections, meta_title, meta_description, og_image, created_at, updated_at FROM pages`;
  const params = [];
  if (status) {
...
router.get('/:slug', async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT id, title, title_ru, title_uz, title_en, title_tr, slug, content, status, sections, meta_title, meta_description, og_image FROM pages WHERE slug = $1`,
    [slug]
  );
...
router.post('/', authenticate, authorize(['admin', 'editor']), async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      INSERT INTO pages (title, title_ru, title_uz, title_en, title_tr, slug, content, status, sections, meta_title, meta_description, og_image)
...
router.put('/:id', authenticate, authorize(['admin', 'editor']), async (req, res) => {
  ...
  `, [title, title_ru || null, title_uz || null, title_en || null, title_tr || null,
      ...
      meta_title || null, meta_description || null, og_image || null, id]);
