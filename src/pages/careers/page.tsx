...
import { careersPageText } from "@/lib/i18n/careers.ts";
...
function localizedPosition(v: Vacancy, lang: LangCode): string {
  const byLang: Record<LangCode, string> = {
    ru: v.position_ru, uz: v.position_uz, en: v.position_en, tr: v.position_tr,
  };
  return byLang[lang] || v.position_ru || v.position_en || v.position;
}

// ─── Why Work ────────────────────────────────────────────────────────────────
const WHY_ICONS = [Globe, TrendingUp, Users, Star];

// ─── Apply form ───────────────────────────────────────────────────────────────
const selectCls =
  "flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50";

function ApplyForm({ vacancies, locations, lang }: { vacancies: Vacancy[]; locations: Location[]; lang: LangCode }) {
  const [submitting, setSubmitting] = useState(false);
  const t = careersPageText[lang];

  const applySchema = z.object({
    fullName: z.string().min(1, t.nameRequired),
    phone: z.string().min(1, t.phoneRequired),
    email: z.string().email(t.emailInvalid),
    vacancyId: z.string().min(1, t.vacancyRequired),
    locationId: z.string().min(1, t.locationRequired),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FormField control={form.control} name="fullName" render={({ field }) => (
          <FormItem><FormLabel>{t.formFullName}</FormLabel><FormControl><Input placeholder={t.fullNamePlaceholder} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="phone" render={({ field }) => (
          <FormItem><FormLabel>{t.formPhone}</FormLabel><FormControl><Input placeholder={t.phonePlaceholder} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="email" render={({ field }) => (
          <FormItem className="sm:col-span-2"><FormLabel>{t.formEmail}</FormLabel><FormControl><Input placeholder="example@gmail.com" {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="vacancyId" render={({ field }) => (
          <FormItem><FormLabel>{t.formVacancy}</FormLabel><FormControl>
            <select className={selectCls} {...field}>
              <option value="">{t.selectVacancy}</option>
              {vacancies.map((v) => (
                <option key={v.id} value={v.id}>{localizedPosition(v, lang)}</option>
              ))}
            </select>
          </FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="locationId" render={({ field }) => (
          <FormItem><FormLabel>{t.formLocation}</FormLabel><FormControl>
            <select className={selectCls} {...field}>
        ...
        <FormField control={form.control} name="experience" render={({ field }) => (
          <FormItem className="sm:col-span-2"><FormLabel>{t.formExperience}</FormLabel><FormControl><Input placeholder={t.experiencePlaceholder} {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        ...
        <Button type="submit" size="lg" disabled={submitting}
          ...
          {submitting ? <><Loader2 className="size-4 animate-spin" /> {t.sendingBtn}</> : <>{t.submitBtn} <Send className="size-4" /></>}
        </Button>
      </form>
    </Form>
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-background py-16 sm:py-20">
        <div className="mx-auto max-w-[1140px] px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut" }}
            ...
            <div>
              <p className="text-xs font-semibold tracking-[0.3em] text-accent uppercase">{t.introEyebrow}</p>
      ...
      <section className="bg-secondary/40 py-16 sm:py-24">
        <div className="mx-auto max-w-[1140px] px-6">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: "easeOut" }} className="text-center">
            <h2 className="font-serif text-3xl font-bold text-primary sm:text-4xl">{t.whyTitle}</h2>
            <p className="mt-3 text-sm text-muted-foreground">{t.whySub}</p>
          </motion.div>
      ...
      <section className="bg-background py-16 sm:py-24">
        <div className="mx-auto max-w-[1140px] px-6">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: "easeOut" }}>
            <p className="text-xs font-semibold tracking-[0.3em] text-accent uppercase">{t.vacanciesLabel}</p>
            <h2 className="mt-2 font-serif text-3xl font-bold text-primary sm:text-4xl">{t.vacanciesTitle}</h2>
            <p className="mt-3 text-sm text-muted-foreground">{t.vacanciesLocation}</p>
          </motion.div>
              ...
              vacancies.map((vac, i) => (
                <motion.div key={vac.id} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06, ease: "easeOut" }}
                  ...
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:items-center sm:gap-4">
                    <span className="font-medium text-foreground">{localizedPosition(vac, lang)}</span>
                  ...
                  <a href="#apply" className="flex shrink-0 items-center gap-1 text-sm font-medium text-primary transition-colors hover:text-accent">
                    {t.applyLink} <ChevronRight className="size-4 transition-transform group-hover:translate-x-0.5" />
                  </a>
                </motion.div>
              ))
            )}
      ...
      <section id="apply" className="bg-secondary/40 py-16 sm:py-24">
        <div className="mx-auto max-w-3xl px-6">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: "easeOut" }} className="text-center">
            <p className="text-xs font-semibold tracking-[0.3em] text-accent uppercase">{t.applyEyebrow}</p>
            <h2 className="mt-2 font-serif text-3xl font-bold text-primary sm:text-4xl">{t.applyTitle}</h2>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }}
            ...
            <ApplyForm vacancies={vacancies} locations={locations} lang={lang} />
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-primary py-16 sm:py-20">
        <div className="mx-auto max-w-[1140px] px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: "easeOut" }}
            ...
            <div>
              <p className="text-xs font-semibold tracking-[0.25em] text-accent uppercase">{t.questionsLabel}</p>
              <h2 className="mt-1 font-serif text-2xl font-bold text-primary-foreground sm:text-3xl">{t.questionsTitle}</h2>
            ...
            <Button size="lg" variant="secondary" className="shrink-0 cursor-pointer" asChild>
              <a href="/contact">{t.contactBtn} <ArrowRight className="size-4" /></a>
            </Button>
          </motion.div>
        </div>
      </section>
