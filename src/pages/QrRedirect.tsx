...
export default function QrRedirect() {
    ...
    return (
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 p-6 text-center">
          <p className="text-muted-foreground text-sm">Не удалось загрузить меню.</p>
          <a
            href={PDF_URL}
            target="_blank"
            rel="noreferrer"
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* PDF progress bar */}
      {!loading && total > 0 && (
        <div className="sticky top-[65px] z-40 bg-background/80 backdrop-blur border-b border-border/60 px-4 py-2 flex items-center justify-between text-xs text-muted-foreground">
          <span className="font-medium text-foreground">Меню MADO</span>
          <div className="flex items-center gap-3">
            <span>{rendered} / {total} стр.</span>
            <a
              href={PDF_URL}
              target="_blank"
              rel="noreferrer"
