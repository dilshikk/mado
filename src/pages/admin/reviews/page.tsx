...
export default function ReviewsPage() {
    ...
    try {
      ...
      setReviews(
        items.map((item) => {
          ...
          return {
            id: String(item.id),
            author: item.author_name ?? "Аноним",
            rating: Number(item.rating) || 0,
            text: item.text ?? "",
            source: item.source ?? "Website",
            date: rawDate.toLocaleDateString("ru-RU", {
              day: "2-digit",
              month: "short",
            }),
            rawDate,
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sort) {
        case "date_asc":    return a.rawDate.getTime() - b.rawDate.getTime();
        case "date_desc":   return b.rawDate.getTime() - a.rawDate.getTime();
  ...
  const setStatus = async (id: string, status: ReviewStatus) => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Не удалось обновить статус отзыва");
    }
  };
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-serif font-bold">Отзывы</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {reviews.filter((r) => r.status === "new").length} новых ·{" "}
            {approved.length} опубликованных
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4 flex items-start gap-4">
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{approved.length} отзывов</p>
          </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center rounded-xl border border-border bg-card p-10 text-muted-foreground">
          <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Загрузка отзывов...
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <div className="flex">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={cn("w-3 h-3", i < rev.rating ? "text-amber-400 fill-amber-400" : "text-muted-foreground/30")}
                            />
                          ))}
                        </div>
                        <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded", SOURCE_COLORS[rev.source] ?? "bg-muted text-muted-foreground")}>
                          {rev.source}
                        </span>
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-border flex-wrap">
                ...
                {rev.status === "hidden" && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-muted text-muted-foreground rounded-lg hover:bg-muted/80 font-medium"
                  >
                    <ExternalLink className="w-3 h-3" /> Восстановить
                  </button>
                )}
                {rev.status === "new" && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-muted text-muted-foreground rounded-lg hover:bg-muted/80 font-medium"
                  >
                    <EyeOff className="w-3 h-3" /> Игнорировать
                  </button>
                )}
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Отзыв от <strong>{rev.author}</strong> будет удалён безвозвратно. Это действие нельзя отменить.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
